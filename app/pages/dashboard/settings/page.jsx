...
import { Lock, Mail, User, Trash2, Loader2, Settings, Shield, Key, AlertTriangle, CheckCircle2, X, Edit3 } from 'lucide-react';
import { getUsername, getUseremail } from '@/app/utils/getUsername';
import { updateUsername, sendPasswordResetEmail, DeleteUser } from '@/app/api/auth.service';
  ...
  <div className={`group bg-gradient-to-br from-white to-gray-50 rounded-3xl p-6 border ${dangerous ? 'border-red-200 hover:border-red-300' : 'border-gray-200 hover:border-indigo-200'} hover:shadow-xl transition-all duration-300`}>
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
      ...
      <button
        onClick={onAction}
        disabled={loading}
        className={`px-4 pt-2 sm:pt-0 py-2 mx-auto sm:mx-0 rounded-xl text-sm sm:text-md font-medium transition-all duration-200 hover:shadow-md flex items-center gap-2 ${dangerous ? 'text-red-600 hover:bg-red-50 hover:text-red-700' : `${actionColor} hover:bg-indigo-50 hover:text-indigo-700`}`}
      >
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (userMail) setUserMail(userMail);
    } catch (e) {
      console.log(e);
    }
  }, []);
  ...
  const handleUsernameUpdate = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        localStorage.setItem('updatedUser', JSON.stringify({ name: formData.newUsername, email: usermail }));
  ...
  const deleteAccount = async () => {
    try{
      const res = await DeleteUser({ email: usermail, password: formData.deletePassword });
      if(res.status == 200){
      ...
      <div className="container mx-auto p-6 max-w-4xl">
        ...
        >
          ...
          <InputField
            ...
            placeholder="Confirm with your current password"
          /> */}
          <p className="text-sm text-gray-600 leading-6"> Currently, the application uses your email as the primary identity to manage users. As a result, you cannot delete this email at the moment. Once support for linking multiple emails or mobile numbers is added, you will be able to manage and delete your account more freely. For further inquiries, please contact the developer. Thank you for using our application. </p>
        </ActionModal>
